$(document).ready(function () {
    // Request geolocation when clicking elements that require it (e.g., Clock In)
    function handleGeoClick(e) {
        var $el = $(this);
        if ($el.data('geoReady')) {
            return; // already has coordinates, let normal handler proceed
        }

        e.preventDefault();
        if (!navigator.geolocation) {
            try { appAlert.error('Geolocation is not supported by this browser.'); } catch (err) { console && console.error(err); }
            return;
        }

        navigator.geolocation.getCurrentPosition(function (pos) {
            $el.attr('data-post-latitude', pos.coords.latitude);
            $el.attr('data-post-longitude', pos.coords.longitude);
            $el.data('geoReady', 1);
            // trigger original click to continue normal ajax flow with coords attached
            $el.trigger('click');
            // clean flag for future clicks
            setTimeout(function () { $el.removeData('geoReady'); }, 0);
        }, function (err) {
            var msg = (typeof AppLanugage !== 'undefined' && AppLanugage.locationPermissionDenied) ? AppLanugage.locationPermissionDenied : 'Location permission denied or unavailable.';
            try { appAlert.error(msg); } catch (e2) { console && console.error(err); }
        }, { enableHighAccuracy: true, timeout: 15000, maximumAge: 0 });
    }

    $('body').on('click', '[data-require-geo]', handleGeoClick);
    $('body').on('click', '[data-action-url*="attendance/log_time"]', handleGeoClick);
