...
class Attendance extends Security_Controller {
    ...
    // Return today's (or selected date) latest per-user logins with geolocation for the Attendance page
    function today_list_data() {
        $this->check_module_availability("module_attendance");

        // appTable sends start_date for daily picker
        $date = $this->request->getPost('start_date');
        $list_data = $this->Attendance_model->get_today_logins($date)->getResult();

        $result = array();
        foreach ($list_data as $row) {
            $user = get_team_member_profile_link($row->user_id, $row->user_name);
            $in_time = $row->in_time ? format_to_time($row->in_time) : "-";
            $out_time = $row->out_time ? format_to_time($row->out_time) : "-";
            $status = ucfirst($row->status);

            $in_loc = "-";
            if ($row->check_in_lat !== null && $row->check_in_lng !== null) {
                $mapUrl = "https://maps.google.com/?q=" . $row->check_in_lat . "," . $row->check_in_lng;
                $in_loc = "<a href='" . $mapUrl . "' target='_blank'>" . app_lang('view') . "</a>";
            }

            $out_loc = "-";
            if ($row->check_out_lat !== null && $row->check_out_lng !== null) {
                $mapUrl2 = "https://maps.google.com/?q=" . $row->check_out_lat . "," . $row->check_out_lng;
                $out_loc = "<a href='" . $mapUrl2 . "' target='_blank'>" . app_lang('view') . "</a>";
            }
    ...
    function log_time($user_id = 0) {
        ...
        $this->Attendance_model->log_time($user_id ? $user_id : $this->login_user->id, $note, $latitude, $longitude);
    ...
    private function _make_clock_in_out_row($data) {
        if (isset($data->attendance_id)) {
            ...
            $view_data = js_anchor("<i data-feather='log-in' class='icon-16'></i> " . app_lang('clock_in'), array('title' => app_lang('clock_in'), "class" => "btn btn-default spinning-btn", "data-action-url" => get_uri("attendance/log_time/$data->id"), "data-action" => "update", "data-inline-loader" => "1", "data-post-id" => $data->id, "data-require-geo" => "1"));
        }
