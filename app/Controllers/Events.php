    ...
    function save() {
        ...
        if ($id && !$this->login_user->is_admin) {
            ...
            if ($event_info->created_by == $this->login_user->id) {
                ...
                $share_with = $event_info->share_with ? explode(",", $event_info->share_with) : array();

                if ($this->login_user->user_type === "staff") {
                    if (in_array("all", $share_with) || in_array("member:" . $this->login_user->id, $share_with)) {
                        $can_edit_event = true;
                    } else if ($this->login_user->team_ids) {
                        $team_ids = explode(",", $this->login_user->team_ids);
                        foreach ($team_ids as $team_id) {
                            if (in_array("team:" . trim($team_id), $share_with)) {
                                $can_edit_event = true;
                                break;
                            }
                        }
                    ...
                    if (in_array("all_contacts", $share_with) || in_array("contact:" . $this->login_user->id, $share_with)) {
                        $can_edit_event = true;
                    }
                }
            }
