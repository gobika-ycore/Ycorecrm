...
class Projects extends Security_Controller {
    ...
    function save() {
        ...
        $posted_project_type = $this->request->getPost('project_type');
        $existing_project = null;
        if ($id) {
            $existing_project = $this->Projects_model->get_one($id);
        }
        $project_type = $posted_project_type ? $posted_project_type : ($existing_project ? $existing_project->project_type : 'client_project');
        ...
        if ($project_type === "internal_project") {
            ...
            $posted_client_id = $this->request->getPost('client_id');
            if ($posted_client_id !== null && $posted_client_id !== "") {
                $data["client_id"] = $posted_client_id;
            } else if ($id && $existing_project) {
                //preserve existing client_id on edit if not posted
                $data["client_id"] = $existing_project->client_id;
            } else if (!$id) {
                //on create client project, require explicit client
                $data["client_id"] = $posted_client_id; //may be empty; validation elsewhere can handle if needed
            }
        }
        ...
        if ($save_id) {
            ...
            //return database error to help diagnose why the update failed
            $db = Database::connect();
            $db_error = $db->error();
            $error_message = (isset($db_error['message']) && $db_error['message']) ? $db_error['message'] : app_lang('error_occurred');
