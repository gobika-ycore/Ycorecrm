    ...
    private function _get_sidebar_menu_items($type = "") {
        ...
        if ($this->ci->login_user->user_type == "staff" && $type !== "client_default") {
            ...
            if (get_setting("module_attendance") == "1" && ($this->ci->login_user->is_admin || $access_timecard)) {
                ...
                // Added Attendance extensions
                $team_submenu["holidays"] = array("name" => "holidays", "url" => "holidays", "class" => "calendar");
                $team_submenu["attendancereports"] = array("name" => "attendancereports", "url" => "attendancereports", "class" => "bar-chart-2");
                ...
                // Added Attendance extensions (read-only access path still allowed to open modules)
                $team_submenu["holidays"] = array("name" => "holidays", "url" => "holidays", "class" => "calendar");
                $team_submenu["attendancereports"] = array("name" => "attendancereports", "url" => "attendancereports", "class" => "bar-chart-2");
            }
